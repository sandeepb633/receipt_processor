...
# *** CRUCIAL CHANGE: Initialize Flask-Session HERE ***
Session(app)
...
def split_bill():
    if request.method == 'POST':
        ...
        print("Debug - Form ", form_data)

        for item in items:
            num_members = int(form_data.get(f'members_{item.description}', 0))
            print(f"Debug - Item: {item.description}, Number of members: {num_members}")
            ...
            if num_members > 0:
                ...
                print(f"Debug - Member names for {item.description}: {member_names}")
        ...
        print("Debug - members_dict:", members_dict)
...
def show_results():
    ...
    print("Debug - session_", session_data)
    ...
    if request.method == 'POST':
        ...
        if action == 'edit_price':
            try:
                ...
                if new_price_str and new_price_str.strip():
                    ...
                    if 0 <= item_index < len(items):
                        ...
                        print(f"Debug - Price updated for item {item_index}: {new_price}")

            except (ValueError, IndexError) as e:
                print(f"Debug - Error updating price: {str(e)}")
...
if __name__ == '__main__':
    app.run(debug=True)  # Remove or comment this out for Render deployment
